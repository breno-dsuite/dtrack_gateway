...
DEBUG_PESAGEM = dados.get('DEBUG_PESAGEM', False)
DEBUG_PESO_PESAGEM = dados.get('DEBUG_PESO_PESAGEM', False)
...
def on_message(ws, message):
    ...
    def pesagem(evento):
        ...
        while not peso:
            ...
            if tentativa == 10:
                break
        if DEBUG_PESO_PESAGEM:
            peso = 1.000
        rs = requests.get(f'https://{HOST}/gateway/pesagem/{pesagem_id}', json={'peso': peso})
        if rs.status_code == 200:
            log_to_file(f"PESAGEM - {pesagem_id} - {peso}")
        else:
            log_to_file(f"PESAGEM ERROR - {pesagem_id} - {peso}")
